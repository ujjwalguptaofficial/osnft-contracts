...
async function main() {
    ...
    const osnftMeta = process.env.OSNFT_META;

    const contract = await ethers.getContractFactory('OSNFTMeta');

    const contractFactory = contract.connect(ledger);

    const contractInstance = await contractFactory.attach(osnftMeta as string);
    ...
    const tokenAddresses = ["0x596F902535152a904B80f6947794084E1b5e7a40"];
    ...
    const tx = await contractInstance.addPayableTokens(tokenAddresses, {
        ...
        maxPriorityFeePerGas: 1500000032
    });
    await tx.wait();
